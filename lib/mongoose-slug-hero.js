...
function SlugHero (schema, options) {
  ...
  // --------- options --------- //
    ...
    slugFieldName = options.slugField || options.slugFieldName || SlugHero.config.slugFieldName,

    // field name for storing slugs (optional / global)
    slugsFieldName = options.slugsField || options.slugsFieldName || SlugHero.config.slugsFieldName,
  ...
  schema.pre('findOneAndUpdate', function (done) {
    var _this = this,
			...
			// New entered slug
			newVal = _.get(updates, slugField)
			
    if (newVal) {

      _this.findOne({}, function (err, old) {								
				
				// Make sure slug were changed 
        if (_.get(old, slugField) == newVal) {
          // Nothing changes
          done()
  ...
  schema.pre('save', true, function (next, done) {
    ...
    if (_this.isNew) {
      ...
      _this.constructor.findById(_this.id, function (err, old) {
        ...
        if (_.get(old, slugField) == _.get(_this, slugField)) {										
          // Nothing changes
          done()
